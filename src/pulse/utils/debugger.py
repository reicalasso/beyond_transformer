...
class PulseDebugger:
    ...
    def __init__(self, log_dir: str = "debug_logs", verbose: bool = True):
        ...
        # Setup logging
        self.logger = logging.getLogger("PulseDebugger")
        self.logger.setLevel(logging.DEBUG if verbose else logging.INFO)

        # Create file handler
        log_file = (
            self.log_dir / f"pulse_debug_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
        ...
        self.logger.info(f"PULSE Debugger initialized. Logs saved to: {log_file}")
    ...
    def log_memory_operation(
        ...
        operation_data = {
            ...
            "read_data": (
                self._serialize_tensor(read_data) if read_data is not None else None
            ),
            "write_data": (
                self._serialize_tensor(write_data) if write_data is not None else None
            ),
            "attention_weights": (
                self._serialize_tensor(attention_weights)
        ...
        self.log_step(
            f"Memory_{operation_type.capitalize()}",
    ...
    def log_attention_operation(
        ...
        attention_data = {
            ...
            "attended_values": (
                self._serialize_tensor(attended_values)
                if attended_values is not None
                else None
            ),
            "operation_info": operation_info or {},
        }
        ...
        self.log_step(
            f"Attention_{attention_type.capitalize()}",
    ...
    def log_state_update(
        ...
        state_data = {
            ...
            "state_diff": (
                self._serialize_tensor(new_state - old_state)
        ...
        self.log_step(f"State_Update_{component_name}", {"state_update": state_data})

        # Log statistics
        if self.verbose and new_state is not None:
            new_state_np = new_state.detach().cpu().numpy()
            self.logger.debug(
                f"State update for {component_name}: "
    ...
    def _serialize_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            if isinstance(value, torch.Tensor):
                ...
                serialized[key] = [
                    (
                        self._serialize_data({"item": item})["item"]
                        if isinstance(item, dict)
                        else (
                            self._serialize_tensor(item)
    ...
    def _serialize_tensor(self, tensor: torch.Tensor) -> Dict[str, Any]:
        ...
        return {
            ...
            "sample": (
                tensor_np.flatten()[:100].tolist() if tensor_np.size > 0 else []
            ),  # First 100 elements
        }
    ...
    def print_summary(self):
        ...
        self.logger.info("=== PULSE Debug Summary ===")
        ...
        memory_ops = [
            entry for entry in self.log_data if "memory_operation" in entry["data"]
        ]
        self.logger.info(f"Memory operations: {len(memory_ops)}")

        # Attention operations summary
        attention_ops = [
            entry for entry in self.log_data if "attention_operation" in entry["data"]
        ]
        self.logger.info(f"Attention operations: {len(attention_ops)}")

        # State updates summary
        state_updates = [
            entry for entry in self.log_data if "state_update" in entry["data"]
        ]
...
class DebuggablePULSEComponent(nn.Module):
    """
    Base class for debuggable PULSE components.
    """
    ...
    def set_debugger(self, debugger: PulseDebugger):
        """
        Set debugger for this component.

        Args:
            debugger: PulseDebugger instance
        """
    ...
    def log_step(
        ...
        if self.debugger is not None:
            self.debugger.log_step(
                f"{self.component_name}_{step_name}", data, step_info
            )
...
if __name__ == "__main__":
    print("Testing PULSE Debugger...")

    # Create debugger
    debugger = PulseDebugger("test_debug_logs", verbose=True)
    ...
    print("✅ PULSE Debugger test completed!")
