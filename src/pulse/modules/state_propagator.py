"""
State Propagator Module for PULSEs
...
class StatePropagator(nn.Module):
    ...
    def _gru_step_multi(self, prev_states, new_inputs):
        ...
        # Reshape for processing all states together - use contiguous().reshape() for safety
        prev_states_flat = prev_states.contiguous().reshape(-1, state_dim)
        new_inputs_flat = new_inputs.contiguous().reshape(-1, state_dim)
        ...
        new_states = new_states.reshape(batch_size, num_states, state_dim)
