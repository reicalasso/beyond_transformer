"""
Debuggable PULSE Components

This module implements debuggable versions of key PULSE components.
"""
...
from pulse.utils.debugger import DebuggablePULSEComponent, PulseDebugger


class DebuggableTokenToStateRouter(DebuggablePULSEComponent):
    """
    ...
    def forward(
        ...
        if self.debug_mode:
            self.log_step("routing_logits_computed", {"routing_logits": routing_logits})
        ...
        if self.debug_mode:
            self.log_step(
                "routing_weights_computed", {"routing_weights": routing_weights}
            )
            self.log_attention_operation(
                "token_to_state",
                f"tokens_{seq_len}",
                f"states_{num_states}",
                routing_weights,
            )
        ...
        if self.debug_mode:
            self.log_step("tokens_routed", {"routed_tokens": routed_tokens})
...
class DebuggableStateManager(DebuggablePULSEComponent):
    """
                ...
                if self.debug_mode:
                    self.log_step(
                        "states_pruned",
                        {
                            ...
                            "pruned_indices": torch.where(low_importance_mask)[
                                0
                            ].tolist(),
                        },
                    )
    ...
    def allocate_states(self, num_states: int) -> int:
        ...
        with torch.no_grad():
            ...
            if self.debug_mode:
                ...
                self.log_memory_operation(
                    "allocate",
                    f"allocated_{num_to_allocate}_states",
                    operation_info={
...
class DebuggableStatePropagator(DebuggablePULSEComponent):
    """
    ...
    def forward(
        self, prev_state: torch.Tensor, new_input: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        if self.debug_mode:
            self.log_step(
                "state_propagation_started",
                {"prev_state": prev_state, "new_input": new_input},
            )
        ...
        if self.debug_mode:
            self.log_step(
                "state_propagation_completed", {"updated_state": updated_state}
            )

        return updated_state

    def _single_state_update(
        self, prev_state: torch.Tensor, new_input: torch.Tensor
    ) -> torch.Tensor:
        """
    ...
    def _multi_state_update(
        self, prev_states: torch.Tensor, new_inputs: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        if self.debug_mode:
            self.log_step(
                "multi_state_update_started",
                {"prev_states": prev_states, "new_inputs": new_inputs},
            )
        ...
        if self.debug_mode:
            self.log_step(
                "multi_state_update_completed", {"updated_states": updated_states}
            )
    ...
    def _gru_step(
        ...
        if self.debug_mode:
            self.log_step(
                "gru_step_started", {"prev_state": prev_state, "new_input": new_input}
            )
    ...
    def _lstm_step(
        ...
        if self.debug_mode:
            self.log_step(
                "lstm_step_started", {"prev_state": prev_state, "new_input": new_input}
            )
    ...
    def _apply_communication(self, states: torch.Tensor) -> torch.Tensor:
        ...
        if self.debug_mode:
            self.log_step(
                "communication_started", {"states_before_communication": states}
            )
...
if __name__ == "__main__":
    print("Testing Debuggable PULSE Components...")

    # Create debugger
    debugger = PulseDebugger("test_component_logs", verbose=True)
    ...
    state_manager = DebuggableStateManager(
        state_dim=128, max_states=16, initial_states=8, debug_mode=True
    )
    ...
    propagator = DebuggableStatePropagator(
        state_dim=128, gate_type="gru", debug_mode=True
    )
    ...
    print(
        f"   Previous state: {prev_state.shape} -> Updated state: {updated_state.shape}"
    )
    ...
    print("\n✅ Debuggable PULSE Components test completed!")
