...
class AdvancedHybridModel(nn.Module):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.input_projection = nn.Linear(
            self.input_dim, self.embedding_dim * self.seq_len
        )
        self.positional_encoding = nn.Parameter(
            torch.randn(self.seq_len, self.embedding_dim)
        )
        ...
        self.ssm_block = SSMBlock(
            ...
            expand=(
                default_config["ssm_dim"] // self.embedding_dim
                if default_config["ssm_dim"] > self.embedding_dim
                else 2
            ),
        )
        ...
        self.read_key_generator = nn.Linear(
            self.embedding_dim, read_heads * ntm_mem_dim
        )
        self.read_strength_generator = nn.Linear(self.embedding_dim, read_heads)

        # Write head parameter generators
        self.write_key_generator = nn.Linear(
            self.embedding_dim, write_heads * ntm_mem_dim
        )
        self.write_strength_generator = nn.Linear(self.embedding_dim, write_heads)
        self.erase_vector_generator = nn.Linear(
            self.embedding_dim, write_heads * ntm_mem_dim
        )
        self.add_vector_generator = nn.Linear(
            self.embedding_dim, write_heads * ntm_mem_dim
        )

        # RNN Memory
        rnn_input_dim = self.embedding_dim + (
            read_heads * ntm_mem_dim
        )  # Attention output + NTM read vectors
        ...
        self.output_projection = nn.Linear(
            default_config["rnn_hidden_dim"], self.output_dim
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        embedded_seq = embedded_seq.view(
            batch_size, self.seq_len, self.embedding_dim
        )  # [batch_size, seq_len, embedding_dim]

        # Add positional encoding
        embedded_seq = embedded_seq + self.positional_encoding.unsqueeze(
            0
        )  # [batch_size, seq_len, embedding_dim]
        embedded_seq = self.embedding_norm(embedded_seq)

        # 2. Attention Processing
        attended_seq, attention_weights = self.attention.forward_self_attention(
            embedded_seq
        )
        ...
        read_keys = self.read_key_generator(ssm_features).view(
            batch_size, self.config["ntm_read_heads"], self.config["ntm_mem_dim"]
        )
        read_strengths = F.softplus(self.read_strength_generator(ssm_features))

        write_keys = self.write_key_generator(ssm_features).view(
            batch_size, self.config["ntm_write_heads"], self.config["ntm_mem_dim"]
        )
        write_strengths = F.softplus(self.write_strength_generator(ssm_features))
        erase_vectors = torch.sigmoid(self.erase_vector_generator(ssm_features)).view(
            batch_size, self.config["ntm_write_heads"], self.config["ntm_mem_dim"]
        )
        add_vectors = torch.tanh(self.add_vector_generator(ssm_features)).view(
            batch_size, self.config["ntm_write_heads"], self.config["ntm_mem_dim"]
        )
        ...
        read_vectors_flat = read_vectors.view(
            batch_size, -1
        )  # [batch_size, num_read_heads * ntm_mem_dim]

        # 5. RNN Processing
        # Combine attention features with NTM read vectors
        rnn_input_features = torch.cat(
            [attention_features, read_vectors_flat], dim=-1
        )  # [batch_size, embedding_dim + num_read_heads * ntm_mem_dim]

        # Expand to sequence for RNN processing
        rnn_input_seq = rnn_input_features.unsqueeze(1).repeat(
            1, self.seq_len, 1
        )  # [batch_size, seq_len, ...]
        ...
        # RNN processing
        rnn_output_seq, _ = self.rnn_memory(
            rnn_input_seq, hidden
    ...
    def get_model_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "parameter_details": {
                "input_projection": sum(
                    p.numel() for p in self.input_projection.parameters()
                ),
...
class SequentialHybridModel(nn.Module):
    ...
    def __init__(self, input_dim: int = 784, output_dim: int = 10):
        ...
        # NTM layer
        self.ntm = NTMMemory(
            mem_size=64, mem_dim=16, num_read_heads=1, num_write_heads=1
        )
        ...
        # RNN layer
        self.rnn = RNNMemory(
            input_dim=self.embedding_dim + 16, hidden_dim=128, rnn_type="gru"
        )
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Input → Sequence
        seq_x = self.input_to_sequence(x).view(
            batch_size, self.seq_len, self.embedding_dim
        )
        ...
        read_strength = F.softplus(
            self.ntm_read_strength(ssm_feature).squeeze(-1)
        )  # [batch_size]
        write_strength = F.softplus(
            self.ntm_write_strength(ssm_feature).squeeze(-1)
        )  # [batch_size]
        erase_vector = torch.sigmoid(self.ntm_erase(ssm_feature)).unsqueeze(
            1
        )  # [batch_size, 1, 16]
        add_vector = torch.tanh(self.ntm_add(ssm_feature)).unsqueeze(
            1
        )  # [batch_size, 1, 16]
        ...
        # RNN
        rnn_input = torch.cat([attention_feature, ntm_feature], dim=-1).unsqueeze(
            1
        )  # [batch_size, 1, embedding_dim+16]
