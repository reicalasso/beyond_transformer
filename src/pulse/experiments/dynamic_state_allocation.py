...
class DynamicPULSE(nn.Module):
    """
    PULSE with dynamic state allocation and pruning.
    """

    def __init__(self, input_dim, state_dim, max_states, output_dim):
        """
        Initialize the DynamicPULSE.
        ...
        """
        super(DynamicPULSE, self).__init__()
        ...
        # PULSE layer
        self.pulse_layer = PulseLayer(state_dim=state_dim, token_dim=state_dim, num_heads=4)

        # Output projection
        self.output_projection = nn.Linear(
            state_dim * 8, output_dim
    ...
    def forward(self, x):
        ...
        # Get current states
        current_states = self.state_memory[
            :, : self.state_manager.get_active_count(), :
        ]
        current_states = current_states.repeat(
            batch_size, 1, 1
        )  # [batch_size, active_states, state_dim]

        # Expand embedded input to match states
        expanded_input = embedded.repeat(
            1, current_states.size(1), 1
        )  # [batch_size, active_states, state_dim]

        # Apply PULSE layer
        updated_states = self.pulse_layer(current_states, expanded_input)
