#!/usr/bin/env python3
"""
Train PULSE v3 on TinyStories.

Usage (defaults — no config file needed):
    python scripts/train.py

Usage (with YAML config):
    python scripts/train.py --config configs/4090.yaml
"""
...
class TinyStoriesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(tok) < self.L + 1:
            tok = tok + [self.pad] * (self.L + 1 - len(tok))
        else:
            tok = tok[: self.L + 1]
...
def get_lr(step, warmup, total, max_lr, min_lr=None):
    min_lr = min_lr or max_lr * 0.1
    if step < warmup:
...
def train(args):
        ...
        if step % args.eval_steps == 0:
            ...
            with torch.no_grad():
                for vx, vy in val_loader:
                    ...
                    vy     = vy.clone(); vy[vy == pad_id] = -100
                    with (autocast("cuda", dtype=autocast_dtype) if autocast_dtype else contextlib.nullcontext()):
    ...
    log.info("=" * 70)
    log.info(f"Done!  {elapsed/3600:.2f}h  best_val={best_val:.4f}  "
             f"tok/s={tokens_seen/elapsed:.0f}")
    log.info("=" * 70)

    # Final samples
    model.eval()
    for prompt in ["Once upon a time", "The little girl", "Tom was a boy who"]:
        s = generate_sample(model, tokenizer, device, prompt, max_tokens=120)
        log.info(f"\n[{prompt}]\n{s}")
...
def _load_yaml(path: str) -> Dict[str, Any]:
    """Load a YAML config file, returning a flat dict of arg_name -> value."""
    try:
        import yaml
    except ImportError:
    ...
    # Flatten any top-level sections (model/training/data) into a single dict
    flat: Dict[str, Any] = {}
    for key, val in data.items():
        if isinstance(val, dict):
            ...
            flat[key] = val
    # Convert hyphenated keys to underscored so they match argparse dest names
    return {k.replace("-", "_"): v for k, v in flat.items()}


def main():
    # First pass: grab --config only, so YAML values become defaults that
    # CLI flags can still override (priority: defaults < YAML < CLI)
    pre = argparse.ArgumentParser(add_help=False)
    pre.add_argument("--config", type=str, default=None)
    pre_args, _ = pre.parse_known_args()

    p = argparse.ArgumentParser(description="Train PULSE v3 on TinyStories")
    p.add_argument("--config", type=str, default=None, metavar="YAML",
                   help="Optional YAML config file; CLI flags override it")
    ...
    p.add_argument("--fast-decay",   type=float, default=0.70)
    p.add_argument("--slow-decay",   type=float, default=0.97)
    ...
    # I/O
    p.add_argument("--output-dir",  type=str, default="./output/pulse_v3")
    p.add_argument("--resume",      type=str, default=None)

    # Apply YAML as defaults before final parse
    if pre_args.config:
