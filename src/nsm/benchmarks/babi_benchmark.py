"""
bAbI Benchmark for Neural State Machine Models
...
class bAbIDataset(Dataset):
    ...
    def __init__(self, task_id: int = 1, size: int = 1000, 
                 max_story_length: int = 20, max_query_length: int = 10):
        """
    ...
    def _generate_babi_data(self) -> Tuple[List[List[int]], List[List[int]], List[int]]:
        ...
        if self.task_id in [1, 2, 3]:  # Supporting facts, two supporting facts, three supporting facts
        ...
        self.word_to_idx = {word: idx + 1 for idx, word in enumerate(sorted(set(all_words)))}
        ...
        for _ in range(self.size):
            ...
            if len(padded_story) < self.max_story_length * 10:
                padded_story.extend([0] * (self.max_story_length * 10 - len(padded_story)))
        ...
        return (torch.tensor(stories, dtype=torch.long), 
...
class bAbIBenchmark:
    ...
    def run_task(self, task_id: int, batch_size: int = 32, 
        ...
        with torch.no_grad():
            for batch_idx, (stories, queries, answers) in enumerate(dataloader):
                stories, queries, answers = (stories.to(self.device), 
    ...
    def run_key_tasks(self, batch_size: int = 32, 
        ...
        key_tasks = [1, 2, 3, 16, 19]  # Supporting facts tasks + induction + path finding
        results = {}
