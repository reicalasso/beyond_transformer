...
class TransformerAttention(nn.Module):
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor,
        ...
        v = v.view(batch_size, -1, self.num_heads, self.d_k).transpose(1, 2)  # [batch_size, num_heads, seq_len_k, d_k]
        ...
        if attention_mask is not None:
            if attention_mask.dim() == 2:
                attention_mask = attention_mask.unsqueeze(0).unsqueeze(0)  # [1, 1, seq_len_q, seq_len_k]
        ...
        context = context.transpose(1, 2).contiguous()  # [batch_size, seq_len_q, num_heads, d_k]
