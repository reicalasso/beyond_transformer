"""
Advanced Visualization Tools for Neural State Machine Components

This module provides specialized visualization tools for NSM-specific components.
"""
...
class AdvancedNSMVisualizer:
    """
    Advanced visualizer for Neural State Machine components.
    """
    ...
    def plot_token_to_state_routing(self, routing_weights: torch.Tensor,
        ...
        if state_labels:
            ...
            ax.set_xticklabels([f"State {i}" for i in range(num_states)], rotation=45, ha="right")
    ...
    def plot_state_communication(self, attention_weights: torch.Tensor,
                               state_labels: Optional[List[str]] = None,
    ...
    def plot_memory_read_write_operations(self, 
        ...
        ax.bar(x - width/2, read_w[:num_slots], width, label='Read', 
               color=self.palette[0], alpha=0.8)
        ax.bar(x + width/2, write_w[:num_slots], width, label='Write',
        ...
        if memory_slots:
            ...
            ax.set_xticklabels([f"Slot {i}" for i in range(num_slots)], rotation=45, ha="right")
    ...
    def plot_state_dynamics(self, state_trajectories: List[torch.Tensor],
                          state_labels: Optional[List[str]] = None,
        ...
        for i, metric in enumerate(metrics):
            ...
            for state_idx in range(num_plot_states):
                ...
                ax.plot(time_steps, values, marker='o', linewidth=2, markersize=4,
                       label=f'State {state_idx}', color=self.palette[state_idx])
    ...
    def create_comprehensive_report(self, visualization_data: Dict[str, Any],
                                  save_dir: str = "visualization_reports") -> str:
        """
        ...
        report_dir = save_path / f"nsm_visualization_report_{timestamp}"
        ...
        readme_content = f"""# NSM Visualization Report
...
This report provides insights into the internal workings of the Neural State Machine.
"""
...
if __name__ == "__main__":
    print("Testing Advanced NSM Visualizer...")
    
    # Create visualizer
    visualizer = AdvancedNSMVisualizer()
    ...
    # Test state dynamics
    state_trajectories = [torch.randn(5, 10) for _ in range(8)]  # 5 states, 10 dims, 8 time steps
    ...
    print("✅ Advanced NSM Visualizer test completed!")