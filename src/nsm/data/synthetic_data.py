"""
Synthetic Data Generators for Neural State Machine Testing

This module provides synthetic data generators for testing NSM models.
"""
...
class SyntheticDataGenerator:
    """
    Generate synthetic data for testing Neural State Machine models.
    """
    
    @staticmethod
    def generate_copy_task(batch_size: int, sequence_length: int, 
                          vocab_size: int = 10) -> Tuple[torch.Tensor, torch.Tensor]:
        """
    ...
    @staticmethod
    def generate_repeat_copy_task(batch_size: int, sequence_length: int, 
                                repeats: int = 2, vocab_size: int = 10) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        target_sequences = torch.cat([
            repeated_sequence,
            end_token,
            torch.zeros(batch_size, sequence_length * repeats + 1, dtype=torch.long)
        ], dim=1)
        
        return input_with_tokens, target_sequences
    
    @staticmethod
    def generate_associative_recall_task(batch_size: int, num_pairs: int, 
                                       key_length: int = 3, value_length: int = 3,
                                       vocab_size: int = 10) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        # Flatten pairs
        pairs_flat = torch.cat([keys.view(batch_size, -1), values.view(batch_size, -1)], dim=1)
        input_seq = torch.cat([
            pairs_flat,
            queries,
        ...
        target_seq = torch.cat([
            targets,
            torch.full((batch_size, 1), vocab_size, dtype=torch.long),
            torch.zeros(batch_size, key_length + value_length + 1, dtype=torch.long)
        ], dim=1)
        
        return input_seq, target_seq
    
    @staticmethod
    def generate_pattern_matching_task(batch_size: int, pattern_length: int = 5,
                                     sequence_length: int = 20, vocab_size: int = 5) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        ...
        embed_pos = torch.randint(0, sequence_length - pattern_length + 1, (batch_size,))
        ...
        target_seq = torch.cat([
            ...
            torch.full((batch_size, 1), vocab_size, dtype=torch.long),
            torch.zeros(batch_size, sequence_length + pattern_length + 1, dtype=torch.long)
        ], dim=1)
...
if __name__ == "__main__":
    ...
    print("\n2. Repeat Copy Task:")
    input_seq, target_seq = SyntheticDataGenerator.generate_repeat_copy_task(batch_size, 3, repeats=2)
    print(f"   Input shape: {input_seq.shape}")
    print(f"   Target shape: {target_seq.shape}")
    print(f"   Sample input: {input_seq[0]}")
    print(f"   Sample target: {target_seq[0]}")
    ...
    input_seq, target_seq = SyntheticDataGenerator.generate_associative_recall_task(batch_size, 3)
    print(f"   Input shape: {input_seq.shape}")
    print(f"   Target shape: {target_seq.shape}")
    print(f"   Sample input: {input_seq[0]}")
    print(f"   Sample target: {target_seq[0]}")
    ...
    input_seq, target_seq = SyntheticDataGenerator.generate_pattern_matching_task(batch_size)
    print(f"   Input shape: {input_seq.shape}")
    print(f"   Target shape: {target_seq.shape}")
    print(f"   Sample input: {input_seq[0]}")
    print(f"   Sample target: {target_seq[0]}")
